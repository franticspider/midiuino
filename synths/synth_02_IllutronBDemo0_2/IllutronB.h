...
/*//////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 also performs the calculations required to generate the sound for each voice/instrument when it is 
 asked by the containing CIllutronB class
 ...
 2) Asking the voices to calculate thier output whenever the synth needs to update the sound - so 
    thats 8,000 times a second !
 3) Performing additional calculations on the voice outputs to generate the sound - scaling them so that
    they can share one 8 bit channel
 4) Generating the output - its incedible, but the entire sound of the synth including the four 
    independent channels is being output through PWM on one 8 bit timer - digital pin 6
 5) Providing a tempo - the user can update the Beats per minute at any time by calling - setBPM - 
    the pitch will not change, just the speed

 Note that a squencer is not currently provided within CIllutronB - it would be a logical development to 
 provide an additional sequencer class It would also be logical to provide a class for storing and
 retreiving voices. But lets get creating first.
