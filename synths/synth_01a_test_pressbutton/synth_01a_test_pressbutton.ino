...
void setup()
{
  ...
  //TODO: check these next settings don't interfere with the above!

  pinMode(PB_MASTER, INPUT);
  ...
  //TODO: should set this to the midi rate eventually...
  Serial.begin(9600);
...
void toggleLEDs() {
  ...
  if (nextled > 3) nextled = 0;

  digitalWrite(leds[onled], LOW);
...
void loop()
{
  ...
  /* Let's check the pushbutton */
  mbstate = digitalRead(PB_MASTER);
  if (mbstate) {
    toggleLEDs();
  }
