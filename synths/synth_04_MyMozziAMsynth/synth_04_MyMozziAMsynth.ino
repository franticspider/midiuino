
/* Based on the following: */
/*  Example of Amplitude Modulation synthesis
    using Mozzi sonification library.
  
    Demonstrates modulating the gain of one oscillator
    by the instantaneous amplitude of another,
    shows the use of fixed-point numbers to express fractional
    values, random numbers with rand(), and EventDelay()
    for scheduling.
  
    Circuit: Audio output on digital pin 9 on a Uno or similar, or
    DAC/A14 on Teensy 3.1, or 
    check the README or http://sensorium.github.com/Mozzi/
  
    Mozzi help/discussion/announcements:
    https://groups.google.com/forum/#!forum/mozzi-users
  
    Tim Barrass 2012, CC by-nc-sa.
*/


// ---------------------------------------------------------------------------
// #defines and #includes:

//#include <ADC.h>  // Teensy 3.1 uncomment this line and install http://github.com/pedvide/ADC
#include <MozziGuts.h>
#include <Oscil.h>
#include <tables/cos2048_int8.h> // table for Oscils to play
//#include <mozzi_utils.h>
#include <mozzi_fixmath.h>
#include <EventDelay.h>
#include <mozzi_rand.h>
#include <mozzi_midi.h>


//#define USING_MIDI

#ifdef USING_MIDI
#include <MIDI.h>
#endif


#define CONTROL_RATE 64 // powers of 2 please

// ---------------------------------------------------------------------------
// public constants

#ifdef USING_MIDI

//These are the notes as defined in "pitches.h", used a lookup table for the midi values:
const int notemap[127] = {
/*Bb0*/   31,
/*C1*/    33,  35,  37,  39,  41,  44,  46,  49,  52,  55,  58,  62, 
/*C2*/    65,  69,  73,  78,  82,  87,  93,  98, 104, 110, 117, 123,
/*C3*/   131, 139, 147, 156, 165, 175, 185, 196, 208, 220, 233, 247,
/*C4*/   262, 277, 294, 311, 330, 349, 370, 392, 415, 440, 466, 494,
/*C5*/   523, 554, 587, 622, 659, 698, 740, 784, 831, 880, 932, 988,
/*C6*/  1047,1109,1175,1245,1319,1397,1480,1568,1661,1760,1865,1976,
/*C7*/  2093,2217,2349,2489,2637,2794,2960,3136,3322,3520,3729,3951,
/*C8*/  4186,4435,4699,4978
};
#endif


// audio oscils (range from -128 to 128)
Oscil<COS2048_NUM_CELLS, AUDIO_RATE> aCarrier(COS2048_DATA);
Oscil<COS2048_NUM_CELLS, AUDIO_RATE> aModulator(COS2048_DATA);
Oscil<COS2048_NUM_CELLS, AUDIO_RATE> aModDepth(COS2048_DATA);


// for scheduling note changes in updateControl()
EventDelay  kNoteChangeDelay;


// synthesis parameters in fixed point formats
Q8n8 ratio; // unsigned int with 8 integer bits and 8 fractional bits
Q24n8 carrier_freq; // unsigned long with 24 integer bits and 8 fractional bits
Q24n8 mod_freq; // unsigned long with 24 integer bits and 8 fractional bits


// for random notes
Q8n0 octave_start_note = 42;


// ---------------------------------------------------------------------------
// functions

void setup(){
  ratio = float_to_Q8n8(3.0f);   // define modulation ratio in float and convert to fixed-point
  kNoteChangeDelay.set(400);//200); // note duration ms, within resolution of CONTROL_RATE
  aModDepth.setFreq(13.f);     // vary mod depth to highlight am effects
  randSeed(); // reseed the random generator for different results each time the sketch runs
  startMozzi(CONTROL_RATE);
}


// ---------------------------------------------------------------------------
// volume control functions

// control variable, use the smallest data size you can for anything used in audio
const byte maxgain=255;
const int maxpot = 1023;
byte volgain = maxgain;
byte gain = 255;

int potpin = 2;
int potval,opv;

void updatevol(){

    opv = potval;
    potval = mozziAnalogRead(potpin);    // read the value from the sensor
    volgain = potval>>2;
    
    if(opv!=potval){
        
      Serial.print("potval is\t");
      Serial.print(potval);
      Serial.print(", volgain =");
      Serial.print(volgain);
      Serial.print("\n");
      
      opv=potval;
    }
}



void updateControl(){

    
#ifdef USING_MIDI

#else

    static Q16n16 last_note = octave_start_note;
  
    if(kNoteChangeDelay.ready()){
  
      // change octave now and then
      if(rand((byte)5)==0){
        last_note = /*36*/24+(rand((byte)6)*12);
      }
  
      // change step up or down a semitone occasionally
      if(rand((byte)13)==0){
        last_note += 1-rand((byte)3);
      }
  
      // change modulation ratio now and then
      if(rand((byte)5)==0){
        ratio = ((Q8n8) 1+ rand((byte)5)) <<8;
      }
  
      // sometimes add a fractionto the ratio
      if(rand((byte)5)==0){
        ratio += rand((byte)255);
      }
  
      // step up or down 3 semitones (or 0)
      last_note += 3 * (1-rand((byte)3));
  
      // convert midi to frequency
      Q16n16 midi_note = Q8n0_to_Q16n16(last_note); 
      carrier_freq = Q16n16_to_Q24n8(Q16n16_mtof(midi_note));
  
      // calculate modulation frequency to stay in ratio with carrier
      mod_freq = (carrier_freq * ratio)>>8; // (Q24n8   Q8n8) >> 8 = Q24n8
  
        // set frequencies of the oscillators
      aCarrier.setFreq_Q24n8(carrier_freq);
      aModulator.setFreq_Q24n8(mod_freq);
  
      // reset the note scheduler
      kNoteChangeDelay.start();
    } 
  }

#endif
  

int updateAudio(){
  /*TODO: Incorporate the gain value in this - need to calculate 
   * what the range should be and scale it accordingly...
   */

  //This can range from 0 to 128^2
  long mod = (128u+ aModulator.next()) * ((byte)128+ aModDepth.next());

  /* apply the modulation to the carrier wave: */
  int out = (mod * aCarrier.next())>>16;

  /* here's where we add the gain: */
  out = (out * gain)>>8;

  return out;
}


void loop(){
  audioHook();
}




